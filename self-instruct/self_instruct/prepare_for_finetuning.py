...
def parse_instances_for_generation_task(raw_text, instruction, response_metadata):
    ...
    if "finish_reason" in response_metadata["response"]["choices"][0]:
        if response_metadata["response"]["choices"][0]["finish_reason"] == "length":
            instances = instances[:-1]
    elif "finishReason" in response_metadata["response"]["choices"][0]:
        if response_metadata["response"]["choices"][0]["finishReason"] == "length":
            instances = instances[:-1]
